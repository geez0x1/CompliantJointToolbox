%BODE2TEST Test implementation of the bode2 plotting functions.
%
...
sysd2           = c2d(sys, dt*10, 'Tustin');

testCase.('TestData').sys   = sys;
testCase.('TestData').sysd  = sysd;
testCase.('TestData').sysd2 = sysd2;
...
function testBasicPlots(testCase)
    % Test specific code

    % Shorthands
    sys     = testCase.('TestData').sys;
    sysd	= testCase.('TestData').sysd;
    sysd2	= testCase.('TestData').sysd2;

    % BODEPLOTS - Magnitude and phase
    figure; hold on;
    bodeOpt             = bodeoptions;
    opt                 = bode2options;
    bodeOpt.XLim        = [0.1 100];        % set the plot limits to minimum and maximum frequency, default would be [1, 10]
    bodeOpt.FreqUnits   = 'Hz';             % change frequency units, default frequency unit is rad/s

    % Actual bodeplot
    bode2(sys, bodeOpt, opt, 'b');
    bode2(sysd, bodeOpt, opt, 'r');
    bode2(sysd2, bodeOpt, opt, 'Color', [0 0.6 0]);
    
    % Legend, title
    bode2legend({'Continuous-time', 'Discrete-time', 'Discrete-time (slow sampling)'}, bodeOpt, opt);
    bode2title(bodeOpt, opt, 0, 'paper mode title', 'normal title');
    
    % BODEPLOTS - Magnitude only
    figure; hold on;
    bodeOpt             = bodeoptions;
    opt                 = bode2options;
    bodeOpt.PhaseVisible= 'off';
    bodeOpt.XLim        = [0.1 100];    % set the plot limits to minimum and maximum frequency, default would be [1, 10]
    bodeOpt.FreqUnits   = 'rad/s';      % change frequency units, default frequency unit is rad/s
    ...
    % Legend, title
    bode2legend({'Continuous-time', 'Discrete-time', 'Discrete-time (slow sampling)'}, bodeOpt, opt);
    bode2title(bodeOpt, opt, 1, 'paper mode title', 'normal title');
